    ...
    showDialog(
      ...
      builder: (ctx) => AlertDialog(
        ...
        actions: [
          ...
          ElevatedButton(
            onPressed: () async {
              if (commentController.text.trim().isEmpty || user == null || profile == null) {
    ...
    try {
      ...
      if (sentiment['riskLevel'] == 'high' || sentiment['riskLevel'] == 'critical') {
        await ref.read(firestoreProvider).flagHighRiskStudent(
          ...
          message: 'Forum post: "${_title.text.trim()}" - ${content.substring(0, (content.length > 100 ? 100 : content.length))}...',
        );
      }
                ...
                child: posts.isEmpty
                    ...
                    : ListView.builder(
                        ...
                        itemBuilder: (context, index) {
                          ...
                          final hasLiked = post.likes.contains(ref.watch(authStateProvider).value?.uid);
                          return Card(
                            child: Padding(
                                ...
                                children: [
                                  ...
                                  Row(
                                    children: [
                                      IconButton(
                                        ...
                                        onPressed: () async {
                                          ...
                                          if (userId != null) {
                                            try {
                                              if (hasLiked) {
                                                await ref.read(firestoreProvider).unlikeForumPost(post.id, userId);
                                              } else {
                                                await ref.read(firestoreProvider).likeForumPost(post.id, userId);
                                              }
                                            } catch (e) {
                                              ScaffoldMessenger.of(context).showSnackBar(
                                                SnackBar(content: Text('Error: $e')),
                                      ...
                                      Text('${post.likes.length} likes', style: Theme.of(context).textTheme.bodySmall),
                                      ...
                                      Text('${post.commentCount} comments', style: Theme.of(context).textTheme.bodySmall),
                                    ],
                                  ),
                                          ...
                                          return Column(
                                            ...
                                            children: [
                                                      ...
                                                      Text(
                                                        DateFormat('MMM d, h:mm a').format(
                                                          DateTime.fromMillisecondsSinceEpoch(
                                                            (comment['createdAt'] as num?)?.toInt() ?? 0,
                                                          ),
                                                        ),
                                                        style: Theme.of(context).textTheme.bodySmall,
                                                      ),
                                              ...
                                              if (comments.length > 3)
                                                TextButton(
                                                  onPressed: () => _showAllComments(context, post),
